...
(function () {
  ...
  // ── Inline the detector (since MV3 content scripts can't import modules easily) ──
  // [detector.js is concatenated here at build time; for dev we duplicate key parts]
  ...
  // ── Input selectors for known AI chat sites ───────────────────────────────
  ...
  let pendingText = "";
  let pendingFindings = [];
  let findingCheckboxes = {}; // ruleId -> {checked, el}

  // ── Attach intercept to submit ────────────────────────────────────────────
  ...
  function handleSubmitAttempt(inputEl) {
    if (!settings.enabled) return true; // let through
    ...
    if (findings.length === 0) return true; // clean, let through

    // Auto-redact mode: silently replace and send
    ...
    // Manual review mode: show overlay
  ...
  // ── Build and show the review overlay ────────────────────────────────────
  ...
  function showOverlay(text, findings) {
    ...
    // Live preview update on checkbox change
  ...
  // ── Force-submit after redaction ──────────────────────────────────────────
  function submitInput(el) {
    if (!el) return;
    // Try clicking the nearest submit/send button
  ...
  function logToSession(findings, originalText) {
    chrome.storage.local.get(["sessionLog"], (res) => {
      ...
      chrome.storage.local.set({ sessionLog: log.slice(-200) }); // keep last 200
    });
  }

  // ── Intercept keyboard submit (Enter / Ctrl+Enter) ─────────────────────────
  function attachKeyboardIntercept(el) {
    ...
    el.addEventListener("keydown", (e) => {
      ...
      const isSubmitCombo = !e.shiftKey && (
        (isMac && e.metaKey) || (!isMac && e.ctrlKey) || el.tagName === "TEXTAREA" || el.getAttribute("contenteditable")
      );
  ...
  // ── DOM observer: watch for dynamically rendered inputs ───────────────────
  function findAndAttach() {
    for (const selector of INPUT_SELECTORS) {
      document.querySelectorAll(selector).forEach((el) => {
        attachKeyboardIntercept(el);

        // Try to find adjacent send button
        const parent = el.closest("form") || el.parentElement;
        if (parent) {
          const sendBtn = parent.querySelector('button[type="submit"], button[aria-label*="send" i], button[data-testid*="send" i], button svg');
          const actualBtn = sendBtn?.closest("button") || sendBtn;
          if (actualBtn) attachButtonIntercept(actualBtn, el);
        }
      });
    }
  }
